...
@catub.cat_cmd(
    ...
    info={
        "header": "To enable or disable sudo of your TempestUB.",
...
async def chat_blacklist(event):
    "To enable or disable sudo of your TempestUB."
...
async def _(event):
    ...
    if len(sudochats) == 0:
        return await edit_delete(
            event, "__There are no sudo users for your TempestUB.__"
        )
    result = "**The list of sudo users for your TempestUB are :**\n\n"
...
async def _(event):  # sourcery no-metrics
    ...
    if input_str[0] == "-all":
        ...
        for plugin in input_str:
            if plugin not in PLG_INFO:
                errors += (
                    f"`{plugin}` __There is no such plugin in your TempestUB__.\n"
                )
            else:
        ...
        for cmd in input_str:
            if cmd not in CMD_INFO:
                errors += f"`{cmd}` __There is no such command in your TempestUB__.\n"
    ...
    result = f"__Successfully enabled __ `{len(loadcmds)}` __ for TempestUB sudo.__\n"
    output = (
...
async def _(event):  # sourcery no-metrics
    ...
    result = f"__Successfully disabled __ `{count}` __ for TempestUB sudo.__\n"
    output = (
        result + "**Bot is reloading to apply the changes. Please wait for a minute**\n"
    )
    if errors != "":
