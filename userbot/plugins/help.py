...
async def grpinfo():
    outstr = "**Plugins in Tempestbot are:**\n\n"
...
async def cmdlist():
    outstr = "**Total list of Commands in your Tempest Bot are :**\n\n"
    category = ["admin", "bot", "fun", "misc", "tools", "utils", "extra"]
    for cat in category:
        plugins = GRP_INFO[cat]
        outstr += f"**{hemojis[cat]} {cat.title()} ** - {len(plugins)}\n\n"
...
@catub.cat_cmd(
    ...
    info={
        "header": "To get guide for TempestUB.",
...
async def _(event):
    ...
    if not input_str:
        ...
        outstr += f"**👩‍💻 Usage : ** `{cmdprefix}help -c <command name>`"
    await edit_or_reply(
        event, outstr, aslink=True, linktext="Total Commands of Tempestbot are :"
    )
...
async def _(event):
    ...
    if found:
        ...
        out = f"I can't find any such command `{cmd}` in Tempestbot"
